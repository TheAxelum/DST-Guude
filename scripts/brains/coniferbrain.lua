...
function CatcoonBrain:OnStart()
    ...
    PriorityNode(
    {
		...
		RunAway(self.inst, "hostile", 5, LEASH_MAX_DIST, function(hunter)
			self.inst.components.locomotor.walkspeed = self.inst.components.locomotor.runspeed
			
			if not self.inst.isrunning then
				self.inst.isrunning = true
				self.inst:DoTaskInTime(2, function(inst)
					self.inst.isrunning = false
					inst.components.locomotor.walkspeed = inst.basespeed
				end)
			end
			return true
		end),
...
--        Leash(self.inst, GetNoLeaderHomePos, LEASH_MAX_DIST, LEASH_RETURN_DIST),
        ...
        Wander(self.inst, function() 
			if self.inst.components.follower.leader then
				return self.inst.components.follower.leader:GetPosition()
			else
